...
% Color Palette definition matching system styling
...
\end{cases}
\end{equation}
where $N_f = 5$ frames ($\approx 170$\,ms), $\theta_{\text{EAR}} = 0.23$, and $T_{\text{cool}} = 0.90$\,s.
...
\end{align}
The tracker confirms front-facing engagement when $|\text{Yaw}| \le 25^\circ$ and $|\text{Pitch}| \le 20^\circ$.
...
\end{cases}
\end{align}
where $\alpha = 0.08$ and $\delta_{\text{deadzone}} = 8$\,px.

\subsection{Dwell-to-Click State Machine}
Dwell interaction tracks the persistence of gaze coordinates within a spatial radius $\mathcal{R}_{\text{dwell}} = 35$\,px over elapsed time:
\begin{equation}
P_{\text{dwell}}(t) = \min\left(1.0, \frac{t - t_{\text{anchor}}}{T_{\text{dwell}}}\right), \quad T_{\text{dwell}} = 1.50\,\text{s}
\end{equation}
When $P_{\text{dwell}}(t) = 1.0$, a system click event is fired and a refractory cooldown $T_{\text{cooldown}} = 1.80$\,s is enforced.
