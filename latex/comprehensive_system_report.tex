...
% =============================================================================
\section{Mathematical Formulations \& Variable Definitions}
% =============================================================================

\subsection{Eye Aspect Ratio (EAR) \& Blink Detection}
Blink events and eye openness are computed using the scalar ratio of vertical eyelid distances to the horizontal eye width:
\begin{equation}
...
\textbf{Variable and Symbol Definitions:}
\begin{itemize}
    \item $P_1 = \text{Outer eye canthus corner point (MediaPipe landmark index 33 for left eye, 362 for right eye)}$
    \item $P_2 = \text{Upper eyelid outer landmark point (index 160 for left eye, 385 for right eye)}$
    \item $P_3 = \text{Upper eyelid inner landmark point (index 158 for left eye, 387 for right eye)}$
    \item $P_4 = \text{Inner eye canthus corner point (index 133 for left eye, 263 for right eye)}$
    \item $P_5 = \text{Lower eyelid inner landmark point (index 153 for left eye, 373 for right eye)}$
    \item $P_6 = \text{Lower eyelid outer landmark point (index 144 for left eye, 380 for right eye)}$
    \item $\|A - B\|_2 = \sqrt{(x_A - x_B)^2 + (y_A - y_B)^2} = \text{2D Euclidean distance between landmark points}$
    \item $\epsilon = 10^{-6} = \text{Numerical epsilon constant added to denominator to prevent division-by-zero}$
    \item $\text{EAR}_L = \text{Instantaneous Eye Aspect Ratio scalar computed for the left eye}$
    \item $\theta_{\text{EAR}} = 0.23 = \text{Threshold ratio below which an eyelid is classified as closed}$
    \item $N_f = 4 \text{ frames } (\approx 133\,\text{ms}) = \text{Consecutive frame persistence required to filter natural micro-blinks}$
    \item $t_{\text{last}} = \text{Timestamp of the last successfully executed click action}$
    \item $T_{\text{cool}} = 0.90\,\text{s} = \text{Refractory cooldown duration preventing unintentional repeated clicks}$
\end{itemize}

\subsection{Head Pose Estimation from Transformation Matrix}
From the $4 \times 4$ rigid transformation matrix $M$ computed by the landmark graph, the $3 \times 3$ rotation submatrix $R \in SO(3)$ yields Euler angles:
\begin{align}
...
\textbf{Variable and Symbol Definitions:}
\begin{itemize}
    \item $M \in \mathbb{R}^{4 \times 4} = \text{Homogeneous 3D rigid transformation matrix computed from facial mesh geometry}$
    \item $R \in \mathbb{R}^{3 \times 3} = \text{3D rotation submatrix containing directional orientation vectors}$
    \item $R_{1,0} = \text{Element at row index 1, column index 0 of the rotation matrix } R$
    \item $R_{0,0} = \text{Element at row index 0, column index 0 of the rotation matrix } R$
    \item $R_{2,0} = \text{Element at row index 2, column index 0 of the rotation matrix } R$
    \item $R_{2,1}, R_{2,2} = \text{Elements at row index 2, columns 1 and 2 of the rotation matrix } R$
    \item $\arctan2(y, x) = \text{Four-quadrant inverse tangent returning angles in range } [-\pi, \pi]$
    \item $\frac{180^\circ}{\pi} = \text{Conversion factor transforming radian angle values into degrees}$
    \item $\text{Yaw} = \text{Horizontal head turn angle (valid interaction range: } |\text{Yaw}| \le 25.0^\circ\text{)}$
    \item $\text{Pitch} = \text{Vertical head tilt angle (valid interaction range: } |\text{Pitch}| \le 20.0^\circ\text{)}$
\end{itemize}
...
\textbf{Variable and Symbol Definitions:}
\begin{itemize}
    \item $I_x, I_y = \text{Horizontal and vertical pixel coordinates of the center of the pupil/iris (landmarks 468, 473)}$
    \item $C_{\text{inner}, x} = \text{Horizontal position of the inner eye canthus (landmark 133)}$
    \item $C_{\text{outer}, x} = \text{Horizontal position of the outer eye canthus (landmark 33)}$
    \item $g_x, g_y = \text{Normalized dimensionless gaze ratios bounded in interval } [0.0, 1.0]$
    \item $[x_{\min}, x_{\max}, y_{\min}, y_{\max}] = [0.35, 0.65, 0.30, 0.70] = \text{Empirical user calibration bounding box}$
    \item $W_{\text{screen}} = \text{Horizontal resolution of the display monitor in pixels (e.g., } 1920\,\text{px)}$
    \item $H_{\text{screen}} = \text{Vertical resolution of the display monitor in pixels (e.g., } 1080\,\text{px)}$
    \item $\text{clip}(v, 0, 1) = \text{Clamping function constraining values: } \max(0.0, \min(1.0, v))$
    \item $s_x, s_y = \text{Calculated raw pixel coordinates mapped to the display window}$
\end{itemize}
...
\textbf{Variable and Symbol Definitions:}
\begin{itemize}
    \item $s_{x, t}, s_{y, t} = \text{Raw calculated gaze screen coordinates at current frame } t$
    \item $\hat{s}_{x, t-1}, \hat{s}_{y, t-1} = \text{Smoothed output screen coordinates from the previous frame } t-1$
    \item $d = \text{Spatial Euclidean translation distance between current gaze point and previous filtered coordinate}$
    \item $\delta_{\text{deadzone}} = 8\,\text{px} = \text{Micro-saccadic tremor deadzone threshold below which movement is ignored}$
    \item $\alpha = 0.08 = \text{Exponential Moving Average smoothing coefficient (weight given to new frame: } 8\%\text{)}$
    \item $1 - \alpha = 0.92 = \text{Inertial smoothing coefficient (weight retained from history: } 92\%\text{)}$
    \item $\hat{s}_t = (\hat{s}_{x, t}, \hat{s}_{y, t}) = \text{Final stabilized, tremor-free coordinate dispatched to the OS mouse cursor}$
\end{itemize}
...
\textbf{Variable and Symbol Definitions:}
\begin{itemize}
    \item $t = \text{Current system clock timestamp in seconds}$
    \item $t_{\text{anchor}} = \text{Timestamp when gaze first entered and stabilized within the current button/zone}$
    \item $T_{\text{dwell}} = 1.50\,\text{s} = \text{Continuous fixation duration required to confirm an intentional click selection}$
    \item $P_{\text{dwell}}(t) = \text{Normalized dwell charging progress ranging continuously from } 0.0 \text{ to } 1.0$
    \item $\mathcal{R}_{\text{dwell}} = 35\,\text{px} = \text{Maximum allowable spatial fixation jitter radius around anchor position}$
    \item $T_{\text{cooldown}} = 1.80\,\text{s} = \text{Post-click lockout cooldown preventing accidental double-clicks on neighboring keys}$
\end{itemize}

% =============================================================================
\section{Current Implementations \& Advancements Over Baseline}
% =============================================================================

This section highlights the specific engineering advancements implemented in the current system and provides a comparative analysis explaining how each change directly improves typing accuracy, reduces misclicks, and ensures rigorous experimental validity over the previous prototype.

\begin{table}[H]
\centering
\caption{Comparative Analysis: Previous Baseline vs. Current Enhanced Implementation.}
\small
\begin{tabularx}{\textwidth}{p{0.22\textwidth} p{0.22\textwidth} p{0.22\textwidth} X}
\toprule
\textbf{Functional Subsystem} & \textbf{Previous Baseline} & \textbf{Current Implementation} & \textbf{Quantitative \& Operational Benefit} \\
\midrule
\textbf{Micro-Jitter Suppression} & Deadzone: $4$\,px \newline EMA $\alpha = 0.12$ & \textbf{Deadzone: $8$\,px} \newline \textbf{EMA $\alpha = 0.08$} & Eliminates involuntary 4--7\,px ocular tremors, preventing the cursor from vibrating across adjacent key borders. \\
\midrule
\textbf{Dwell-to-Click Timing} & Hold: $1.25$\,s \newline Radius: $40$\,px \newline Cooldown: $1.50$\,s & \textbf{Hold: $1.50$\,s} \newline \textbf{Radius: $35$\,px} \newline \textbf{Cooldown: $1.80$\,s} & Resolves the ``Midas Touch'' problem by allowing reading without premature clicking; restricts spatial dwell strictly to target keys. \\
\midrule
\textbf{Cursor Origin Alignment} & Arbitrary cursor position at trial onset & \textbf{Automated Center Warping (\texttt{\_center\_cursor})} & Uses Win32 \texttt{SetCursorPos} to place cursor at $(W/2, H/2)$ at start and post-rest, standardizing Cognitive Latency (CLPI). \\
\midrule
\textbf{Stimulus Standardization} & Random unweighted sentence sampling & \textbf{Deterministic 8-Trial Canonical Pool} & Uniform 5-word + 3-pangram progression guarantees $100\%$ repeatable conditions across all participants. \\
\midrule
\textbf{On-Screen Layout Diversity} & Fixed single QWERTY keyboard & \textbf{QWERTY, Alphabetical, Frequency-Clustered} & Enables rigorous multi-layout comparative HCI research measuring Fitts' law motor targeting and visual search. \\
\midrule
\textbf{Ocular Fatigue Mitigation} & Continuous typing without enforced breaks & \textbf{30\,s Rest Screen with TTS Audio Guidance} & Suspends video capture and announces rest/resumption via audio, preventing gaze drift caused by dry/strained eyes. \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{In-Depth Technical Rationale of Key Advancements}

\subsubsection{1. Ocular Tremor and Micro-Saccade Suppression}
In human vision, the eyes exhibit continuous micro-tremors (drift and micro-saccades) spanning $4$ to $7$ pixels on standard displays. In the previous configuration ($\delta_{\text{deadzone}} = 4$\,px, $\alpha=0.12$), these involuntary micro-movements caused the cursor to jitter erratically between neighboring keys. By expanding the deadzone to $8$\,px and tuning the EMA filter to $\alpha=0.08$, micro-tremors are filtered out entirely while purposeful eye movements smoothly transition the cursor with minimal lag.

\subsubsection{2. Anti-Midas Touch Dwell State Machine}
The classic ``Midas Touch'' problem occurs when looking at an interface element to inspect it inadvertently triggers a click. Increasing $T_{\text{dwell}}$ from $1.25$\,s to $1.50$\,s provides a comfortable visual comprehension buffer. Furthermore, tightening the spatial bounding radius from $40$\,px to $35$\,px ensures that shifting gaze to an adjacent letter immediately resets the dwell timer, preventing accidental misclicks.

\subsubsection{3. Automated Screen-Center Cursor Warping}
In eye-tracking experiments, initial cursor location heavily biases first-key latency and trajectory. Through the integration of the \texttt{\_center\_cursor()} routine using Windows API \texttt{SetCursorPos($W/2, H/2$)}, the cursor is automatically repositioned to the exact display center prior to Trial 1 and after every 30-second break, eliminating spatial bias and protecting against edge misclicks.

\subsubsection{4. Standardized Interleaved Stimulus Protocol}
Previously, variable-length prompts resulted in inconsistent cognitive load. The current protocol implements a canonical 8-trial sequence comprising 5 common words and 3 full pangrams arranged in an interleaved progression:
\begin{equation}
W_1 \rightarrow W_2 \rightarrow P_1 \rightarrow W_3 \rightarrow W_4 \rightarrow P_2 \rightarrow W_5 \rightarrow P_3
\end{equation}
This ensures that every subject experiences an identical progression of lexical complexity and motor path lengths.
...
\textbf{Metric Variable Definitions:}
\begin{itemize}
    \item $|T| = \text{Total character length of the final typed transcript produced by the participant}$
    \item $|K| = \text{Total count of registered key selections (including letter keystrokes, spaces, and backspaces)}$
    \item $S = \text{Total duration of the typing trial in seconds from first keystroke to submission}$
    \item $I = \text{Ground-truth target stimulus prompt string presented to the user}$
    \item $\text{Levenshtein}(T, I) = \text{Minimum number of single-character insertions, deletions, or substitutions required}$
    \item $t_{k_i} = \text{Exact timestamp in seconds when the } i\text{-th keystroke was registered}$
    \item $t_{\text{stimulus\_onset}} = \text{Exact timestamp when the typing phase became active on screen}$
\end{itemize}
