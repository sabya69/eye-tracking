...
class Launcher(tk.Tk):
    ...
    def _start_tracker(self):
        ...
        user_name = simpledialog.askstring("Name", "Please enter your name for calibration:", parent=self)
        if not user_name:
            self._flash("Calibration cancelled. Name is required.")
            return

        script = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "_run_accuracy.py")
        with open(script, "w") as f:
            f.write(f"from tracker import AttentionTracker\n"
                    f"t = AttentionTracker(user_name={repr(user_name)})\n"
                    f"t.calibrate(duration=3)\n"
                    f"t.run()\n")
